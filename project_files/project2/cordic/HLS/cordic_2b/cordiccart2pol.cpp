...
void cordiccart2pol(data_var_t x, data_var_t y, data_var_t * r,  data_var_t * theta)
{
    ...
    data_var_t tmp;
    tmp = (y < 0) ? (data_var_t)(-y) : (data_var_t)(y);
    y = (y < 0) ? (data_var_t)(x) : (data_var_t)(-x);
    x = tmp;
    ...
    for (i = 0; i < NO_ITER; i++) {
        ...
        x_shift = y * sigma * (data_var_t)Kvalues[i];
        y_shift = x * sigma * (data_var_t)Kvalues[i];
        x = x - x_shift;
        y = y + y_shift;

        ans_theta = ans_theta - sigma*((data_var_t)(angles[i]));

        debug_printf("Results so far i : %0d, theta : %0f, x: %0f, y: %0f\n", i, ans_theta, x, y);
    }

    *r = x * (data_var_t)0.607252935;
    *theta = ans_theta;
