...
def exercise_5b(input0, output):
    ...
    tap1 = TensorAccessPattern(
        tensor_dims, offset=0, sizes=[1, 2, 2, 4], strides=[0, 24, 12, 1]
    )
    tap2 = TensorAccessPattern(
        tensor_dims, offset=4, sizes=[1, 2, 2, 4], strides=[0, 24, 12, 1]
    )
    tap3 = TensorAccessPattern(
        tensor_dims, offset=8, sizes=[1, 2, 2, 4], strides=[0, 24, 12, 1]
    )

    # Create a TensorTileSequence from a list of taps
    taps = TensorAccessSequence.from_taps([tap1, tap2, tap3])
...
def main():
    ...
    for index, (actual, ref) in enumerate(
        zip(
            output,
            [l * 4 + k * 24 + j * 12 + i for l in range(3) for k in range(2) for j in range(2) for i in range(4)],
        )
    ):
        if actual != ref:
            print(f"Error in output {index}, {actual} != {ref}")
            errors += 1
        else:
            print(f"Correct output {index}, {actual} == {ref}")
