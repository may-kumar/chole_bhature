...
def dense(input0, input1, output):
    # Use only tile sizes
    ...
    worker = Worker(core_fn, [fifo_A_L2L1.cons(), fifo_B_L2L1.cons(), fifo_C_L1L2.prod(), dense_kernel])
...
def main():
    ...
    dense1 = np.matmul(input0.numpy(), input1.numpy())
    ref_vec = np.maximum(dense1,0)
    
    dense(input0, input1, output)
