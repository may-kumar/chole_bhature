...
void dense(const DTYPE *__restrict A, const DTYPE *__restrict B, DTYPE *__restrict C) {
  ...
  for (unsigned row = 0; row < m / r; row += 2) {
    for (unsigned col = 0; col < n / t; col += 2) {
      ...
      auto zeros = aie::zeros<DTYPE, MMUL::size_C>();
    
      // 3. Apply ReLU: Output = Max(Input, 0)
      C00_i16 = aie::max(C00_i16, zeros);
      C01_i16 = aie::max(C01_i16, zeros);
      C10_i16 = aie::max(C10_i16, zeros);
      C11_i16 = aie::max(C11_i16, zeros);
